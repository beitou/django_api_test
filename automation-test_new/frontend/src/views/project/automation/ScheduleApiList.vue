<template>
    <section>
        <router-link :to="{ name: '计划列表', params: {project_id: this.$route.params.project_id}}" style='text-decoration: none;color: aliceblue;'>
            <el-button class="return-list"><i class="el-icon-d-arrow-left" style="margin-right: 5px"></i>计划列表</el-button>
        </router-link>
        ...
        <el-table :data="ApiList" highlight-current-row v-loading="listLoading" style="width: 100%;" row-key="index"
      align="left">
            ...
            <el-table-column prop="address" label="接口地址" min-width="45%" sortable show-overflow-tooltip>
                <template slot-scope="scope">
...
<script>
        ...
        methods: {
            ...
            updateApiListIndex(){
                let self = this;
                 $.ajax({
                    type: "post",
                    url: test+"/api/automation/update_api_list_index",
                    async: true,
                    data: JSON.stringify({
                        project_id: Number(this.$route.params.project_id),
                        schedule_id: Number(this.$route.params.schedule_id),
                ...
                $.ajax({
                    ...
                    success: (data) => {
                        if (data.code === '999999') {
                            ...
                            // self.result["responseData"] = (data.data.responseData === "")? data.data.responseData : JSON.parse(data.data.responseData.replace(/'/g, "\"").replace(/None/g, "null").replace(/True/g, "true").replace(/False/g, "false").replace(/xa0/g,""));
                            self.result["responseData"] = JSON.parse(data.data.responseData.replace(/'/g, "\"").replace(/None/g, "null").replace(/True/g, "true").replace(/False/g, "false").replace(/xa0/g,""));
                            self.result["testTime"] = data.data.testTime;
                            console.log(self.result.responseData);
    ...
    rowDrop() {
      ...
      const _this = this
        ...
        onEnd({ newIndex, oldIndex }) {
          const currRow = _this.ApiList.splice(oldIndex, 1)[0];
          _this.ApiList.splice(newIndex, 0, currRow);
          _this.updateApiListIndex()
        }
      })
    },
    // //列拖拽
    ...
    // }
        ...
        mounted() {
            ...
            this.schedule = this.$route.params.schedule_id;
