    ...
    <section>
        ...
        <el-col :span="24" style="height: 46px">
            <el-form :inline="true" :model="filters" @submit.native.prevent>
                <el-form-item>
                    <el-input v-model.trim="filters.name" placeholder="名称" @keyup.enter.native="getScheduleList"></el-input>
                </el-form-item>
                <el-form-item>
                ...
                <el-form-item>
                    <el-button type="primary" @click.native="DownloadApi">下载计划</el-button>
                </el-form-item>
        ...
        <el-dialog title="定时任务" :visible.sync="taskVShow"  :close-on-click-modal="false" style="width: 100%;">
            <el-form ref="form" :model="form" label-width="100px" :rules="formRules">
                <el-row :gutter="40">
                    <el-col :span="10">
                        <el-form-item label="计划名称:" prop="scriptName">
                            <el-input v-model="editForm.scheduleName" disabled></el-input>
                        </el-form-item>
                    </el-col>

                    <el-col :span="10">
                        <el-form-item label="接口分组:" prop="firstGroup">
                            <el-select v-model="addForm.firstGroup" placeholder="分组" disabled>
        ...
        <el-dialog title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false" style="width: 65%; left: 17.5%">
            <el-form :model="editForm"  :rules="editFormRules" ref="editForm" label-width="80px">
                <el-form-item label="名称" prop="scheduleName">
                    <el-input v-model.trim="editForm.scheduleName" auto-complete="off"></el-input>
                </el-form-item>
        ...
        <el-dialog title="新增" :visible.sync="addFormVisible" :close-on-click-modal="false" style="width: 65%; left: 17.5%">
            <el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
                <el-form-item label="名称" prop="scheduleName">
                    <el-input v-model.trim="addForm.scheduleName" auto-complete="off"></el-input>
                </el-form-item>
        ...
        <!--列表-->
        <el-table :data="Schedule" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
            <el-table-column type="selection" min-width="5%">
            </el-table-column>
            <el-table-column prop="scheduleName" label="计划名称" min-width="30%" sortable show-overflow-tooltip>
                <template slot-scope="scope">
                    <el-icon name="scheduleName"></el-icon>
                    <router-link :to="{ name: '计划接口列表', params: {schedule_id: scope.row.id}}" style='text-decoration: none;'>{{ scope.row.scheduleName }}</router-link>
                </template>
            </el-table-column>
            ...
            <el-table-column label="操作" min-width="35%">
                <template slot-scope="scope">
                    ...
                    <el-button type="info" size="small" @click="handleEdit(scope.$index, scope.row)">修改</el-button>
                     <el-button type="warning" size="small" @click="getTask(scope.$index, scope.row)">定时任务</el-button>
                    <el-button type="primary" size="small" @click.native="TestReport(scope.$index, scope.row)"><div>查看报告</div></el-button>
                </template>
            </el-table-column>
        </el-table>
    ...
    export default {
        ...
        methods: {
            ...
            DownloadApi() {
                $.ajax({
                    type: "get",
                    url: test+"/api/automation/DownloadSchedule",
                    async: true,
            ...
            getTask: function (index, row){
                let self = this;
                this.getScheduleGroup();
                console.log(index);
                this.form.scheduleId=row.id;
                ...
                this.$refs.form.validate((valid) => {if (valid) {
                    this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            let param = {
                                ...
                                unit: self.form.unit,
                                scheduleId:self.form.scheduleId,
                            };
            ...
            getScheduleList() {
                ...
                $.ajax({
                    type: "get",
                    url: test+"/api/automation/schedule_list",
                    async: true,
                    data: param,
                    headers: {
                        Authorization: 'Token '+JSON.parse(sessionStorage.getItem('token'))
            ...
            updateGroupSubmit() {
                ...
                this.$confirm('确认修改所属分组吗？', '提示', {
                    ...
                    $.ajax({
                        type: "post",
                        url: test+"/api/automation/update_schedule_group",
                        async: true,
                        data: params,
                        headers: {
                            "Content-Type": "application/json",
                        ...
                        success: function(data) {
                            ...
                            if (data.code === '999999') {
                                ...
                                self.$router.push({ name: '分组计划列表', params: { project_id: self.$route.params.project_id, firstGroup: self.updateGroupForm.firstGroup}});
                            }
                            else {
                                self.$message.error({
                                    message: data.msg,
            ...
            handleDel: function (index, row) {
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    $.ajax({
                        type: "post",
                        url: test+"/api/automation/del_schedule",
                        async: true,
                        data: JSON.stringify({
                                project_id: Number(this.$route.params.project_id),
                                ids: [row.id] }),
                    ...
                    if (valid) {
                        ...
                        this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            let param = JSON.stringify({
                                ...
                                scheduleName: self.addForm.scheduleName,
                                description: self.addForm.description });
                            $.ajax({
                                type: "post",
                                url: test+"/api/automation/add_schedule",
                                async: true,
                                data: param,
                                headers: {
                                    Authorization: 'Token '+JSON.parse(sessionStorage.getItem('token'))
