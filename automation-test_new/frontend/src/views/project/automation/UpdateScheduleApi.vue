<template>
    <section>
        <router-link :to="{ name: '计划接口列表', params: {project_id: this.$route.params.project_id,schedule_id: this.$route.params.schedule_id}}" style='text-decoration: none;color: aliceblue;'>
        ...
        <el-form :model="form" ref="form" :rules="FormRules">
            <div style="border: 1px solid #e6e6e6;margin-bottom: 10px;padding:15px">
                ...
                <el-form-item label="Host:" label-width="83px" prop="host_id">
                    <el-select v-model="form.host_id" placeholder="请选择host" style="width:30%">
                        <el-option v-for="(item,index) in hosts" :key="index+''" :label="item.name" :value="item.id"></el-option>
                    </el-select>
                </el-form-item>
                <el-row :gutter="10">
                    <el-col :span="5">
                    ...
                    <el-col :span="3" v-if="form.request4!=='DUBBO'">
                        <el-form-item>
                    ...
                    <el-col :span="24" v-show="true" >
                        <el-form-item label="参数解析:" label-width="83px">
                            <el-input
                              type="textarea"
                              :rows="2"
                              placeholder="请输入参数，会自动同步到参数列表"
                              v-model="textarea"
                            @change="analysiData">
                            </el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span='9' v-if="form.request4==='DUBBO'">
                        <el-form-item label="方法:" label-width="83px" prop="dubboMethod">
                            <el-input v-model.trim="form.dubboMethod" placeholder="DUBBO方法名称" auto-complete></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
            </div>
                    ...
                    <el-collapse-item title="请求参数" name="2">
                        <div style="margin: 5px">
                            <el-row :span="24">
                                ...
                                <el-button :class="ParameterTyep? 'parameter-b': 'parameter-a'"  type="primary" size="mini" style="margin-bottom: 5px"  @click="handleCorrelation(null,null,form.parameterRaw)">关联</el-button>
                                <el-col v-if="request3" :span="16"><el-checkbox v-model="radioType" label="3" v-show="ParameterTyep">表单转源数据</el-checkbox></el-col>
                            </el-row>
                        </div>
                        <el-table ref="multipleParameterTable" :data="form.parameter" highlight-current-row :class="ParameterTyep? 'parameter-a': 'parameter-b'">
                        ...
                        <el-card class="box-card">
                            <div slot="header" class="clearfix">
                                <el-radio-group v-model="form.check">
                                    ...
                                    <el-radio-button label="Schema_check"><div>Schema校验</div></el-radio-button>
                                </el-radio-group>
                            ...
                            <div v-show="showCheck">
                                ...
                                <el-group  v-if="form.check==='Schema_check'"  v-model="form.checkSchemaJson">
                                    <el-button  v-bind:style="{marginLeft:'10px'}" @click.native="checkjsontext(false)" label="json_validate"><div>校验/格式化</div></el-button>
                                    <el-button v-bind:style="{marginLeft:'10px'}" @click.native="checkjsontext(true)" label="json_compress"><div>压缩</div></el-button>
                                  </el-group>
                                <el-input class="mock_text" style="margin-top: 10px" v-model.trim="form.checkData" type="textarea" :rows="8" placeholder="请输入mock内容"></el-input>
                            </div>
                        </el-card>
                    </el-collapse-item>
                </el-collapse>
            ...
            <el-row :gutter="10">
                ...
                <el-col :span="18">
                    ...
                    <div style="height:400px;line-height:100px;overflow:auto;overflow-x:hidden;">
                        <el-form :model="interrelateForm" ref="interrelateForm" :rules="InterrelateFormRules">
                            <el-form-item label="参数类型:">
                                <el-select v-model="interrelateForm.interrelateType" placeholder="参数类型">
                                    <el-option v-for="(item,index) in ParameterType" :key="index+''" :label="item.label"
                                               :value="item.value"></el-option>
                                </el-select>
                            </el-form-item>

                            <el-form-item label="选择接口response:">
                                <el-input type="textarea" v-model="interrelateForm.interrelateResponse"
                                          placeholder="接口response"></el-input>
                            </el-form-item>

                            <el-form-item label="辅助生成关联参数" prop="interrelateTxt_temp">
                                <el-input type="textarea" v-model="interrelateForm.interrelateTxt_temp"
                                          placeholder="关联参数，选择接口后自动生成"></el-input>
                            </el-form-item>

                            <el-form-item label="接口参数" prop="interrelateTxt">
                                <el-input type="textarea" v-model="interrelateForm.interrelateTxt"
                                          :disabled=interrelateForm.interrelateDisabled placeholder="请输入参数值"></el-input>
                            </el-form-item>



                            <!--<div v-model="interrelateForm.interrelateTooltip" style="">-->
                                <!--{{interrelateForm.interrelateTooltip }}-->
                            <!--</div>-->
                        </el-form>
...
<script>
    import { test, getHost } from '../../../api/api'
    import $ from 'jquery'
    export default {
        data() {
            return {
                ...
                ParameterType: [{value: 'JSON', label: 'JSON'},
                    {value: 'INT', label: 'INT'},
                ...
                FormRules: {
                    ...
                    host_id : [{ required: true, message: '请输入host', trigger: 'blur' }],
                    addr : [{ required: true, message: '请输入地址', trigger: 'blur' }],


                },
                interrelateForm:{
                    interrelateType:'JSON',
                    interrelateTxt:'',
                    interrelateTxt_temp:'',
                    interrelateResponse:'',
                    interrelateTooltip:"请选择关联接口方法",
                    interrelateDisabled:false,
                },
                InterrelateFormRules:{
                    interrelateTxt : [{ required: true, message: '请输入接口参数', trigger: 'blur' }],
                },
            }
        },
        methods: {
            ...
            handleCorrelation(index, row,parameterRaw) {
                let self = this;
                $.ajax({
                    type: "get",
                        ...
                        if (data.code === '999999') {
                            if (data.data.length) {
                                ...
                                if(row!=null){
                                    ...
                                    self.interrelateForm.interrelateTxt=self.interrelateObjects.value
                                }else {
                                    self.interrelateObjects = parameterRaw;
                                    self.interrelateForm.interrelateTxt=parameterRaw;
                                }
            ...
            handleResponse(index) {
                // this.interrelateForm.interrelateDisabled=false;
                // this.interrelateForm.interrelateTooltip="[JSON]类型，使用[data.token];<br/>[DUBBO]类型，使用逗号分隔[data,token]";
                this.ApiResponse = this.ApiList[index];
                this.interrelateForm.interrelateTxt_temp="${["+this.ApiResponse['id']+"]["+this.ApiResponse['name']+"]["+this.interrelateForm.interrelateType+"][替换为你的参数路径]}";
                if (this.ApiResponse['test_result'] == null){
                    this.interrelateForm.interrelateResponse = "该方法未执行过，无法获取接口返回，手动执行一次后可正常显示";

                }else {
                    this.interrelateForm.interrelateResponse = this.ApiResponse['test_result']['responseData'];
                }

                console.log(this.ApiResponse)

            },
            addInterrelateSubmit: function () {
                this.$refs.interrelateForm.validate((valid)  => {
                    if (valid) {
                        this.saveCorrelation = true;
                try {
                    this.interrelateObjects['value'] =  this.interrelateForm.interrelateTxt
                } catch (e) {
            ...
            analysiData(val){
                ...
                if (temp.indexOf("?")>0) {
                    ...
                    this.form.apiAddress = val.split("?")[0].split("//")[1].substring(val.split("?")[0].split("//")[1].indexOf("/"));
                    ...
                    a.forEach(
                        a => {
                            ...
                            this.form.parameter.push(requestList);
                        }
                    );
                    ...
                    this.toggleParameterSelection(this.form.requestList);
                }
                    ...
                    if (valid) {
                        ...
                        this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            let param = JSON.stringify({
                                ...
                                host_id: self.form.host_id,
                                automationTestSchedule_id: Number(self.$route.params.schedule_id),
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    if (data.code === '999999') {
                                        self.$router.push({ name: '计划接口列表', params: { project_id: self.$route.params.project_id, schedule_id: self.$route.params.schedule_id}});
                                        self.$message({
                                            message: '修改成功',
                                            center: true,
                                            type: 'success'
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.code === '999999') {
                            ...
                            self.form.host_id = data.host_id;
                            self.form.dubboMethod = data.dubboMethod;
                            if (data.formatRaw) {
                                self.radioType = true
                            }
           ...
           checkjsontext(compress){
             const mock_text = this.form.checkData;
              var indentChar = ' ';
              if(/^\s*$/.test(mock_text)){
              alert('数据为空,无法格式化! ');
              return;
