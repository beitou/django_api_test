        ...
        <el-form :model="form" ref="form" :rules="formRules">
            <div style="border: 1px solid #e6e6e6;margin-bottom: 10px; padding:15px; padding-bottom: 0px">
                ...
                <el-row :gutter="10">
                    ...
                    <el-col :span='12' v-if="form.request4==='DUBBO'">
                        <el-form-item prop="dubboMethod">
        ...
        methods: {
                ...
                $.ajax({
                    ...
                    success: (data) => {
                        if (data.code === '999999') {
                            ...
                            self.form.host_id = data.data.host;
                            self.form.dubboMethod = data.data.dubboMethod;
                            ...
                            try {
                                self.form.parameterRaw = data.data.requestParameterRaw.data;
                            } catch (e) {
            ...
            AddHistroy(code) {
                ...
                let param = JSON.stringify({ project_id: Number(this.$route.params.project_id),
                    ...
                    requestAddress: self.form.addr,
                    httpCode: code,
                    protocol: self.form.Http4,
                    host_id: self.form.host_id,
                    dubboMethod: self.form.dubboMethod,
                    ...
                    if (valid) {
                        ...
                        if (self.form.parameterRaw && _type === "raw") {
                            if (!self.isJsonString(self.form.parameterRaw)) {
                                ...
                                this.getRequestResult(url,headers,_parameter,_type,self.form.request4);
                                // $.ajax({
            ...
            getRequestResult(url,header,parameter,request_parameter_type,request_type){
                // if (this.form.request4.toLowerCase()==="get"){
                //     this.requestUrl(url,header,parameter,request_parameter_type);
                // } else if(this.form.request4.toLowerCase()==="post"){
                    this.requestUrl_post(url,header,parameter,request_parameter_type,request_type);
                // }
            },
            requestUrl(url,header,parameter,request_parameter_type){
                let self = this;
                ...
                let params ={
                    parameter_type :request_parameter_type,
                    url:url,
                    protocol: self.form.Http4,
                    parameter:parameter,
                    host_id: self.form.host_id,
                ...
                requrl(headers, params).then(_data => {
                    ...
                    this.AddHistroy(code)
                })
            },
            requestUrl_post(url,header,parameter,request_parameter_type,request_type){
                let self = this;
                // parameter['data']=parameter;
