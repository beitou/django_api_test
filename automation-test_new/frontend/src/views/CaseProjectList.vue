    ...
    <section>
        ...
        <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
            <el-form :inline="true" :model="filters" @submit.native.prevent>
                <el-form-item>
                    <el-input v-model="filters.name" placeholder="名称" @keyup.enter.native="getCaseProjectList"></el-input>
                </el-form-item>
                <el-form-item>
                    <el-button type="primary" @click="getCaseProjectList">查询</el-button>
                </el-form-item>
                <el-form-item>
        ...
        <el-table :data="caseProject" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
            ...
            <el-table-column prop="name" label="项目业务划分" min-width="46%" sortable show-overflow-tooltip>
                <template slot-scope="scope">
                    <el-icon name="name"></el-icon>
                    <router-link v-if="scope.row.status" :to="{ name: '需求项目概况', params: {caseProject_id: scope.row.id}}" style='text-decoration: none;color: #000000;'>
                        {{ scope.row.name }}
                    </router-link>
...
<script>
    ...
    import { getCaseProject, delCaseProject, disableCaseProject, enableCaseProject,
        updateCaseProject, addCaseProject} from '../api/api';
    ...
    export default {
        ...
        methods: {
            ...
            getCaseProjectList() {
                ...
                getCaseProject(headers, params).then((res) => {
            ...
            handleDel: function (index, row) {
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    delCaseProject(header, params).then(_data => {
            ...
            handleChangeStatus: function(index, row) {
                let self = this;
                this.listLoading = true;
                let params = { caseProject_id: row.id};
                let headers = {
                    "Content-Type": "application/json",
                    Authorization: 'Token '+JSON.parse(sessionStorage.getItem('token'))
                };
                if (row.status) {
                    disableCaseProject(headers, params).then(_data => {
                        let { msg, code, data } = _data;
                        self.listLoading = false;
                        if (code === '999999') {
                            self.$message({
                    ...
                    enableCaseProject(headers, params).then(_data => {
                        let { msg, code, data } = _data;
                        self.listLoading = false;
                        if (code === '999999') {
                            self.$message({
                ...
                this.$refs.editForm.validate((valid) => {
                    if (valid) {
                        this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            //NProgress.start();
                            let params = {
                                caseProject_id: self.editForm.id,
                            ...
                            updateCaseProject(header, params).then(_data => {
                                let {msg, code, data} = _data;
                ...
                this.$refs.addForm.validate((valid) => {
                    if (valid) {
                        ...
                        this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            addCaseProject(header, params).then(_data => {
                                let {msg, code, data} = _data;
