	...
	<el-row class="container">
		<el-col :span="24" class="header">
			<el-col :span="10" class="logo" :class="collapsed?'logo-collapse-width':'logo-width'">
				<router-link to="/caseProjectList" style='text-decoration: none;color: #FFFFFF;'>{{collapsed?'':sysName}}</router-link>
			</el-col>
			...
			<template :index='caseProject_id'>
				<el-menu :default-active="$route.path" class="el-menu-vertical-demo" mode="horizontal" @select="handleselect"
					...
					<template v-for="item in $router.options.routes" >
                        <template v-if="item.isCaseProject">
						<template v-for="(items,index) in item.children">
							<el-menu-item :index="items.path" v-if="items.leaf" :key="items.path">
								<template v-if="!items.child">
									<router-link :to="{ name: items.name, item: {id: caseProject_id}}" style='text-decoration: none;color: #000000;'>
										<div>
											{{items.name }}
										</div>
									</router-link>
								...
								<template v-if="items.child">
									<router-link :to="{ name: items.children[0].name, item: {id: caseProject_id,a:'test'}}" style='text-decoration: none;color: #000000;'>
										<div>
											{{items.name }}
										</div>
									</router-link>
