...
class Write:
    ...
    def write_schedule(self, data=None):
        if not data:
            return True
        row = 1
        schedule_row = 1
        ...
        for i in data:
            for api in i["automationGroup"]:
                ...
                if row-1 != schedule_row:
                    self.worksheet.merge_range(schedule_row, 2, schedule_row + len(api["api"]) - 1, 2, api["scheduleName"], merge_format)
                else:
                    self.worksheet.write(schedule_row, 2, api["scheduleName"], row_format)
                schedule_row = row
