...
from api_test.serializers import ProjectDynamicDeserializer, CaseProjectDynamicDeserializer
...
from api_test.models import AutomationTestResult, AutomationScheduleApi, AutomationResponseJson, \
        ...
        if isinstance(dst_data, dict):
            ...
            for key in src_data:
                if key not in dst_data:
                    ...
                    elif isinstance(type(src_data[this_key]), type(dst_data[this_key])) and src_data[this_key] == dst_data[this_key]:
                        result = 'fail'
                    else:
                        pass
            return result
...
def record_results(_id, url, request_type, header, parameter, host,
                   status_code, examine_type, examine_data, _result, code, response_data,start_time=datetime.datetime.now(),is_auto=False):
    """
    记录手动测试结果
    :param _id: ID
    :param url:  请求地址
    ...
    if not is_auto:
        rt = AutomationTestResult.objects.filter(automationScheduleApi=_id)
        if rt:
            rt.update(url=url, requestType=request_type, header=header, parameter=parameter, host=host,
                      ...
                      result=_result, httpStatus=code, responseData=response_data,testTime=start_time)

        else:
            result_ = AutomationTestResult(automationScheduleApi=AutomationScheduleApi.objects.get(id=_id), host=host,
        ...
        record_auto_results(_id, start_time, header, parameter, _result, header, code, response_data)
...
def record_auto_results(_id, time,  header, parameter, _result, responseHeader, code, response_data):
    ...
    :return:
    """
    result_ = AutomationScheduleTestResult(automationScheduleApi=AutomationScheduleApi.objects.get(id=_id), header=header,
...
def create_json(api_id, api, data):
    ...
    # if isinstance(data, dict):
        # for i in data:
        #     m = (api+"[\"%s\"]" % i)
    AutomationResponseJson(automationScheduleApi=api_id, name=api_id, tier=api, type='json').save()
...
def record_caseDynamic(caseProject, _type, operationObject,  user, data):
    """
    记录动态
    :param project: 项目ID
    :param _type: 类型
    ...
    """
    time = datetime.datetime.now()
    dynamic_serializer = CaseProjectDynamicDeserializer(
        data={
            "time": time,
            "caseProject": caseProject, "type": _type,
            "operationObject": operationObject, "user": user,
            "description": data
        }
    )
