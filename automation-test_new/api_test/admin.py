...
from api_test.models import Project, GlobalHost, ApiGroupLevelFirst, ApiInfo, \
    ...
    AutomationGroupLevelFirst, AutomationTestSchedule, AutomationParameter, AutomationScheduleApi, \
...
class AutomationTestScheduleForm(admin.ModelAdmin):
    search_fields = ('scheduleName', 'project')
    list_display = ('id', 'project', 'scheduleName', 'updateTime')
    list_display_links = ('id', 'scheduleName', 'project')
    list_filter = ('project',)
    list_per_page = 20
    ordering = ('id',)
    fieldsets = ([
        '计划接口列表', {
            'fields': ('project', 'automationGroupLevelFirst',
                       'scheduleName', 'user', 'description')
        }],)


admin.site.register(AutomationTestSchedule, AutomationTestScheduleForm)


class AutomationParameterInSchedule(admin.TabularInline):
    model = AutomationParameter


class AutomationHeadInSchedule(admin.TabularInline):
    model = AutomationHead


class AutomationRawInSchedule(admin.TabularInline):
    model = AutomationParameterRaw


class AutomationResponseJsonInSchedule(admin.TabularInline):
    model = AutomationResponseJson


class AutomationScheduleApiForm(admin.ModelAdmin):
    inlines = [AutomationHeadInSchedule, AutomationParameterInSchedule, AutomationRawInSchedule, AutomationResponseJsonInSchedule]
    search_fields = ('automationTestSchedule', 'name', 'apiAddress')
    list_display = ('id', 'automationTestSchedule', 'name', 'httpType', 'requestType', 'apiAddress', 'examineType')
    list_display_links = ('id', 'automationTestSchedule', 'name', 'httpType')
    list_filter = ('httpType', 'requestType', 'requestParameterType', 'examineType')
    list_per_page = 20
    ordering = ('id',)
    fieldsets = ([
        '接口详情', {
            'fields': ('automationTestSchedule', 'name', 'httpType', 'requestType', 'apiAddress',
                       'requestParameterType', 'formatRaw', 'examineType', 'httpCode', 'responseData')
        }],)


admin.site.register(AutomationScheduleApi, AutomationScheduleApiForm)


class AutomationParameterForm(admin.ModelAdmin):
    fieldsets = ([
        '参数详情', {
            'fields': ('automationScheduleApi', 'key', 'value', 'interrelate')
        }],)


class AutomationTestResultForm(ReadOnlyModelAdmin):
    search_fields = ('automationScheduleApi',)
    list_display = ('id', 'automationScheduleApi', 'result', 'httpStatus', 'testTime')
    list_display_links = ('id', 'automationScheduleApi', 'result')
    list_filter = ('id', 'httpStatus', 'result')
    list_per_page = 20
    ordering = ('id',)
    fieldsets = ([
        '测试结果', {
            'fields': ('automationScheduleApi', 'testTime', 'url', 'requestType', 'header', 'parameter', 'statusCode',
...
class AutomationTestTaskForm(admin.ModelAdmin):
    search_fields = ('project', 'name')
    list_display = ('id', 'project', 'Schedule', 'name', 'type', 'startTime', 'endTime')
    list_display_links = ('id', 'project',  'name')
    list_filter = ('type',)
    list_per_page = 20
    ordering = ('id',)
    fieldsets = ([
          '测试任务', {
                'fields': ('project', 'name', 'type', 'frequency',
