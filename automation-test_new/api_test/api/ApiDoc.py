...
from api_test.models import Project, ApiGroupLevelFirst, ApiInfo, \
    ApiOperationHistory, APIRequestHistory, ApiHead, ApiParameter, ApiResponse, ApiParameterRaw, GlobalHost
...
class AddApi(APIView):
    ...
    def parameter_check(self, data):
        ...
        try:
            # 校验必传参数
            if not data["project_id"] or not data["name"] or not \
            ...
            with transaction.atomic():  # 执行错误后，帮助事物回滚
                ...
                if serialize.is_valid():
                    try:
                        ...
                        serialize.save(project=obj, apiGroupLevelFirst=obi, host=obk)
                    except KeyError:
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            ...
            if serialize.is_valid():
                ...
                ApiHead.objects.filter(api_id=data['id']).exclude(header).delete()
                api_param = Q()
                api_param_raw = Q()
                if len(data.get("requestList")):
                    if data["requestParameterType"] == "form-data":
                ...
                ApiParameter.objects.filter(api_id=data['id']).exclude(api_param).delete()
                ApiParameterRaw.objects.filter(api_id=data['id']).exclude(api_param_raw).delete()
                api_response = Q()
                if len(data.get("responseList")):
                    for i in data["responseList"]:
                        if i.get("api") and i.get("id"):
                ...
                ApiResponse.objects.filter(api_id=data['id']).exclude(api_response).delete()
                record_dynamic(project=data["project_id"],
                               _type="新增", operationObject="接口", user=request.user.pk,
                               data="新增接口“%s”" % data["name"])
...
class AddHistory(APIView):
    ...
    def post(self, request):
        ...
        if data["requestType"] == "DUBBO":
            data['requestAddress'] = data['requestAddress'] + ':' + data['dubboMethod']
        else:
            data['requestAddress'] = data['protocol'] + "://" + host_value + data['requestAddress']
...
def download_doc(request):
    ...
    excel_name = url.split("/")
    file_name = str(excel_name[-1])
